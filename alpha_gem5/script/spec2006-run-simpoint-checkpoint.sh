...
if [[ "$ARGC" < 4 ]]
then
	echo "./xxx.sh 400.perlbench(400, perlbench) number(test=0, ref=1,2...) (arm riscv mips alpha amd64) simpoint_index(0,1,2...)"
    exit
fi
# ************************** spec 2006 parameters ******************************

#after restore checkpoint, it simulates instruction number = max_inst（100M）
...
if [ $2 = "0" ]; then
	exe_dir=run_base_test_amd64.0001
fi
...
# ****************************** gem5 parameters  ******************************
gem5_dir=/home/lishuoke/gem5
gem5_config=configs/example/se.py
simpoint_index=$4
gem5_args="--maxinsts=$max_inst \
		   --cpu-type=NonCachingSimpleCPU  --mem-type=SimpleMemory \
		   --restore-simpoint-checkpoint -r $simpoint_index "
...
simpoint_checkpoint_dir=/home/cuihongwei/gem5/simpoint_checkpoint/$target/$ISA-$target-"input"$2
gem5_output_dir=/home/cuihongwei/gem5/run_simpoint_result/$target/$ISA-$target-"input"$2

#--restore-simpoint-checkpoint -r <N> --checkpoint-dir <simpoint checkpoint path>
gem5_args=$gem5_args" --checkpoint-dir=$simpoint_checkpoint_dir"
...
$gem5_dir/build/$gem5_arch \
	--outdir=$gem5_output_dir  $gem5_DebugFlag \
