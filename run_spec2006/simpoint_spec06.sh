...
if [[ "$ARGC" < 3 ]]
then
	echo "./xxx.sh 400.perlbench(400, perlbench) number(test=0, ref=1,2...) (arm riscv mips alpha amd64)"
    exit
fi
...
spec_dir=/home/lishuoke/spec2006
exe_prefix="build_peak_${ISA}.0001"

# ****************************** gem5 parameters  ******************************
gem5_dir=/home/chw/pubs/pubs-gem5
gem5_config=configs/example/se.py

debug_flag="--debug-flags=ShowRunning"
max_inst=900000000000 # 900 billion
simpoint_interval=100000000 #100 million

gem5_output_dir=/home/chw/gem5/simpoint_$ISA
#gem5_args="--simpoint-profile --simpoint-interval=$simpoint_interval \
#			--maxinsts=$max_inst --cpu-type=AtomicSimpleCPU \
#		   --fastmem --mem-size=8GB"
gem5_args="--simpoint-profile --simpoint-interval=$simpoint_interval \
			--maxinsts=$max_inst --cpu-type=NonCachingSimpleCPU \
...
#************************************ run Simpoint **************************************/
echo "****************************** Simpoint ****************************" | tee -a $script_out
simpoint_file=${gem5_output_dir}/simpoint_file
weight_file=${gem5_output_dir}/weight_file
echo "simpoint_dir:   "$simpoint_dir | tee -a $script_out
echo "simpoint_file:  "$simpoint_file | tee -a $script_out
echo "weight_file:    "$weight_file | tee -a $script_out

$simpoint_dir/bin/simpoint -loadFVFile ${gem5_output_dir}/simpoint.bb.gz -maxK 30 -saveSimpoints $simpoint_file -saveSimpointWeights $weight_file -inputVectorsGzipped | tee -a $script_out
